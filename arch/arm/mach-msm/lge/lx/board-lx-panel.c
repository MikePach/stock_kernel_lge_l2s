...
#elif defined(CONFIG_FB_MSM_MIPI_HITACHI_R69324A_VIDEO_WVGA_PT)
...
static int mipi_dsi_panel_power(int on)
{
	...
	if (!dsi_power_on) {
		...
		rc = regulator_set_voltage(reg_l8, 3000000, 3000000); //sangyeol.ryu@lge.com 2011.12.20
		if (rc) {
			pr_err("set_voltage l8 failed, rc=%d\n", rc);
			return -EINVAL;
		}
	...
	defined(CONFIG_FB_MSM_MIPI_R61529_CMD_HVGA_PT))
static int mipi_dsi_panel_power(int on)
{
	...
	if (on) {
		...
		rc = gpio_request(R61529_IFMODE_GPIO, "LCD_IFMODE_GPIO");
		if (rc) {
			pr_err("'%s'(%d) gpio_request failed, rc=%d\n",
				"LCD_IFMODE_GPIO", R61529_IFMODE_GPIO, rc);
			gpio_free(R61529_IFMODE_GPIO);
		}
		rc = gpio_direction_output(R61529_IFMODE_GPIO, 1);
		mdelay(5);
#endif
		rc = regulator_enable(reg_l2);
		if (rc) {
...
#if defined(CONFIG_FB_MSM_MIPI_R61529_VIDEO_HVGA_PT)
		rc = gpio_direction_output(R61529_IFMODE_GPIO, 0);
		gpio_free(R61529_IFMODE_GPIO);
#endif
...
static char backlight_ctrl1[6] = {
	0xB8, 0x01, 0x1A, 0x18,
	0x02, 0x04
};

static char backlight_ctrl4[4] = {
	0xBB, 0x0B, 0xFF, 0x01
};
#endif
...
#if defined(CONFIG_LGE_BACKLIGHT_CABC)
	{DTYPE_GEN_LWRITE, 1, 0, 0, 0, sizeof(backlight_ctrl1),
		backlight_ctrl1},
	{DTYPE_GEN_LWRITE, 1, 0, 0, 0, sizeof(backlight_ctrl4),
		backlight_ctrl4},
#endif
#if defined (DX11D_GAMMA_WB)
	/* Gamma setting */
...
#elif defined (HITACHI_R69324A_GAMMA)
...
static char Set_POWER2 [20] = {0xb1, 0x00, 0x00, 0x04, 0xe8, 0x9a, 0x10, 0x11, 0xb2, 0x0f, 0x2b, 0x33, 0x22, 0x22, 0x41, 0x00, 0x3a, 0xf7, 0x20, 0x80};
static char Set_DISPLAY[ 6] = {0xb2, 0x00, 0x00, 0x78, 0x05, 0x03};
static char Set_VCOM   [ 3] = {0xb6, 0x00, 0xaf};
static char Set_CLK    [ 3] = {0xCB, 0x07, 0x07};
static char Set_PANEL  [ 2] = {0xCC, 0x0A};
static char Set_CYC    [24] = {0xb4, 0x00, 0x06, 0x00, 0x32, 0x10, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x37, 0x0a, 0x40, 0x02, 0x37, 0x0a, 0x40, 0x14, 0x46, 0x50, 0x0a};
...
static char Set_DGC_LUT[128]= {};
static char Set_GAMMA  [35] = {0xE0, 0x05, 0x11, 0x14, 0x37, 0x3F, 0x3F, 0x1D, 0x46, 0x05, 0x0E, 0x0D, 0x13, 0x16, 0x14, 0x17, 0x11, 0x17, 0x05, 0x11, 0x14, 0x37, 0x3F, 0x3F, 0x1D, 0x46, 0x05, 0x0E, 0x0D, 0x13, 0x16, 0x14, 0x17, 0x11, 0x17};
static char exit_sleep [2]  = {0x11, 0x00};
static char display_on [2]  = {0x29, 0x00};
static char enter_sleep[2]  = {0x10, 0x00};
static char display_off[2]  = {0x28, 0x00};
