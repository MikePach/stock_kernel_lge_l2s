...
/*********************************************************************************/
/* LGE ADD                                                                                                                                               */
/* INC_UINT16		m_ucRFTbl                                                                                                       */
/* T39FX  Select RF Table : T39F0 or T39F1                                                                                              */
/*********************************************************************************/
INC_UINT16		m_ucRFTbl		= 0;


void              INC_MUST_DELAY(INC_UINT16 uiDelay)
{
	tdmb_t39fx_must_mdelay(uiDelay);
...
{          
	...
	if(tdmb_t39fx_mdelay(uiDelay) == INC_ERROR)
	{
...
INC_UINT16 INC_I2C_READ(INC_UINT8 ucI2CID, INC_UINT16 uiAddr)
{
	...
	AckStatus= tdmb_t39fx_i2c_read_burst(uiAddr, acBuff,2);	//for android compile
	if(AckStatus == TRUE){
		wData = ((INC_UINT16)acBuff[0] << 8) | (INC_UINT16)acBuff[1];
		return wData;
	}
...
INC_UINT8 INC_I2C_WRITE(INC_UINT8 ucI2CID, INC_UINT16 uiAddr, INC_UINT16 uiData)
{
	...
	AckStatus = tdmb_t39fx_i2c_write_burst(uiAddr, acBuff, ucCnt);	//for android compile
	if(AckStatus == INC_SUCCESS)
	{
		return INC_SUCCESS;
	}
...
INC_UINT8 INC_I2C_READ_BURST(INC_UINT8 ucI2CID,  INC_UINT16 uiAddr, INC_UINT8* pData, INC_UINT16 nSize)
{
	...
	AckStatus = tdmb_t39fx_i2c_read_burst(uiAddr, pData, nSize);
	if(AckStatus == INC_SUCCESS)
	{
		return INC_SUCCESS;
	}
...
INC_UINT16 INC_SPI_REG_READ(INC_UINT8 ucI2CID, INC_UINT16 uiAddr)
{
	...
	INC_INTERRUPT_LOCK();
	tdmb_t39fx_spi_write_read(auiTxBuff, 6, auiRxBuff, 0);
	INC_INTERRUPT_FREE();
...
INC_UINT8 INC_SPI_READ_BURST(INC_UINT8 ucI2CID, INC_UINT16 uiAddr, INC_UINT8* pBuff, INC_UINT16 wSize)
{
	...
	for(uiLoop = 0; uiLoop < nIndex; uiLoop++){
		...
		tdmb_t39fx_spi_write_read(auiBuff, 4, m_auiRxBuff, anLength[uiLoop]);
...
INC_UINT16 INTERFACE_ISR(INC_UINT8 ucI2CID, INC_UINT8* pBuff)
{
	...
	/* modifying read data in remained in T39FX FIFO BUFFER aligned 188 */
	if( unData & 0x4000 ){
		//bFirstLoop = 1;
	...
	/* pBuffer size is 8120. Refer to  TDMB_MPI_BUF_SIZE defined in broadcast_t39fx_drv_if.c */
