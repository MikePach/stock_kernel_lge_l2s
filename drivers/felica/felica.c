 ...
 *    FUNCTION PROTOTYPE
 */
static void felica_delayed_wake_work_func(struct work_struct *data);
static DECLARE_DELAYED_WORK(felica_delayed_wake_work, felica_delayed_wake_work_func);
/*
 *   FUNCTION DEFINITION
 */
 /*
...
static void felica_delayed_wake_work_func(struct work_struct *data)
{
  FELICA_DEBUG_MSG("[FELICA] felica_wake_work_func() is called \n");
...
static int felica_release (struct inode *inode, struct file *fp)
{
  ...
  schedule_delayed_work(&felica_delayed_wake_work,msecs_to_jiffies(1000));
